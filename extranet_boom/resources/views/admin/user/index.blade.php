    ...
    <div class="ibox">
      <div class="ibox-content text-right clearfix">
        <div class="col-md-8 text-left">
          <h2 class="items-counter">{{ sizeof($users) }} usuarios encontrados</h2>
        </div>
        <div class="col-md-4">
          <a href="#" class="btn btn-outline btn-success btn-rounded">Exportar</a>
          <a class="btn btn-outline btn-primary btn-rounded" href="{{ url('/admin/usuarios/nuevo') }}">
        ...
        <table class="footable table table-stripped" data-page-size="50" data-filter=#filter>
          ...
          <tbody>
            @foreach($users as $user)
             @if($user->id > 1)
             <tr class="">
              <td><a href="{{ url('admin/usuarios/actualizar') }}/{{ $user->id }}">{{ $user->name }}</a></td>
              <td>{{ $user->email }}</td>
              <td>
                @if($user->kind == 2)
                Consultor
